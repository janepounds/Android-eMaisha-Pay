...
class TransactionsRemoteMediator(
    ...
    override suspend fun load(loadType: LoadType, state: PagingState<Int, Transactions>): MediatorResult {
        ...
        try {
            val apiResponse = service.getpagedTransactions(wallet_id, page, state.config.pageSize)

            val transactions = apiResponse
